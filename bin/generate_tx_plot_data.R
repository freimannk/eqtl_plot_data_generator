...
option_list <- list(
  ...
  make_option(c("-t", "--tx_gtf_file"), type="character", default=NULL,
              help="Path to the GTF file to get exons of tx transcripts", metavar = "type"),
...
if (FALSE) {
  opt = list()
  opt$n = "Alasoo_2018"
  opt$f = "/Users/kerimov/Work/temp_files/txrevise_stuff/Alasoo_2018_3Apr/susie/Alasoo_2018_tx_macrophage_IFNg.purity_filtered.txt.gz"
  opt$s = "/Users/kerimov/Work/GitHub/SampleArcheology/studies/cleaned/Alasoo_2018.tsv"
  opt$p = "/Users/kerimov/Work/temp_files/txrevise_stuff/Alasoo_2018_3Apr/transcript_usage_Ensembl_105_phenotype_metadata.tsv.gz"
  ...
  opt$t = "/Users/kerimov/Work/temp_files/txrevise_stuff/Alasoo_2018_3Apr/gencode.v39.annotation.nochr.gtf"
  opt$u = "/Users/kerimov/Work/temp_files/txrevise_stuff/Alasoo_2018_3Apr/qcnorm/Alasoo_2018.macrophage_IFNg_tx.tsv.gz"
  index = 1
}
...
message("######### tx_gtf_file_path   : ", tx_gtf_file_path)
...
for (index in 1:nrow(highest_pip_vars_per_cs)) {
  ...
  tx_quant_pheno_ids <- phenotype_metadata %>% dplyr::filter(quant_id %in% ss_oi$quant_id)
  tx_exons <- make_transcript_exon_granges(gff = gtf_ref_tx, transcript_ids = tx_quant_pheno_ids$phenotype_id)
  tx_exons_cdss <- make_transcript_exon_granges(gff = gtf_ref_tx, ss_oi$molecular_trait_id)
  
  exons_to_plot = tx_exons
  exon_cdss_to_plot = tx_exons_cdss
