...
include { generate_plot_majiq_data } from  '../modules/generate_plot_data'
...
workflow recap_plot_majiq {
    ...
    grouped_ge_dir_paths = generate_plot_majiq_data.out.plot_data_directories.groupTuple(by: [0,1,2])
    writeFileFromChannel_ge_input_ch = grouped_ge_dir_paths.map{tuple ->
       def name = tuple[0]
        def method = tuple[1]
